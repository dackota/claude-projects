...
description: Grilling session that challenges a plan against the existing domain model and updates CONTEXT.md/ADRs as decisions crystallise. Use when the user wants to stress-test a plan against their project's language and documented decisions.
...
`otlp_endpoint`/`service_name` if known — leave blank otherwise). That flag is what
