...
#      header (agent/task/verdict∈{PASS,BLOCK,SKIP}/critical/high/rework, plus a
#      `reason` when verdict is SKIP). A prose-only run entry is rejected, so the
#      rework cap + rollup can read fields; a reasonless SKIP is rejected too.
  ...
  awk '
    function flush() {
      if (isrun) {
        ...
        # A SKIP verdict must state WHY the gate was skipped — a reasonless skip is the
        # "silent skip" the barrier forbids (next/BARRIER.md).
        if (fv && vv == "SKIP" && !frn) miss = miss "reason(required-for-SKIP) "
        if (miss != "")
    ...
    /^-[[:space:]]/ {
      flush()
      isrun = 0; fa = 0; ft = 0; fv = 0; fc = 0; fh = 0; fr = 0; frn = 0; vv = ""
      id = "?"
    ...
    /^[[:space:]]+reason:[[:space:]]*[^[:space:]]/   { frn = 1 }
...
if [[ -n "$problems" ]]; then
  {
    echo "Malformed 'run' journal entries — the barrier audit schema requires agent/task/verdict(PASS|BLOCK|SKIP)/critical/high/rework as structured fields (plus a 'reason' when verdict is SKIP; the prose summary does not substitute). Fix before stopping:"
    echo "$problems"
  } >&2
  exit 2
fi
