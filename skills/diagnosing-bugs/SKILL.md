...
mental model of the relevant modules, and check the ADRs in `docs/adr/` for the
...
at least once** (paste the invocation and its output), and that is:

- [ ] **Red-capable** — it drives the actual bug code path and asserts the
  **user's exact symptom**, so it can go red on this bug and green once fixed. Not
  "runs without erroring" — it must be able to _catch this specific bug_.
...
- [ ] The failure is reproducible across multiple runs (or, for non-deterministic
  bugs, reproducible at a high enough rate to debug against).
...
hypotheses they've already ruled out. Cheap checkpoint, big time saver. Don't block
...
1. **Debugger / REPL inspection** if the env supports it. One breakpoint beats ten
   logs.
...
occurs at the call site. (Seam vocabulary — "one adapter = hypothetical seam, two
adapters = real seam" — is defined in the `codebase-design` skill,
`.claude/skills/codebase-design/SKILL.md`.) If the only available seam is too
shallow (single-caller test when the bug needs multiple callers, unit test that
can't replicate the chain that triggered the bug), a regression test there gives
false confidence.

**If no correct seam exists, that itself is the finding.** Note it. The codebase
architecture is preventing the bug from being locked down. Flag this for Phase 6.

If a correct seam exists, drive it through the `tdd` skill's red-green loop:
...
off to the `improve-codebase-architecture` skill
(`.claude/skills/improve-codebase-architecture/`) with the specifics. Make the
